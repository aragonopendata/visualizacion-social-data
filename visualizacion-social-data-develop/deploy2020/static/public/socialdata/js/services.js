angular
  ...
  .factory("escuchaAPI", function ($http, settings) {
    ...
    escuchaAPI.getLastItems = function (
      terms,
      region,
      start,
      end,
    ...
    escuchaAPI.getWeeklyCloud = function (terms, region, start, end) {
      start = new Date(
        start.replace(/^(\d{1,2}\/)(\d{1,2}\/)(\d{4})$/, "$2$1$3")
      );
      ...
      start.getDay() == 0
        ...
        ? (end = new Date(end.setDate(end.getDate() + 7 - end.getDay())))
        : (end = end);
      ...
      for (let week = 0; week < diffCompleteWeeks; week++) {
        ...
        var weekName =
          startWeek.getDate().toString().padStart(2, 0) +
          "/" +
          (startWeek.getMonth() + 1).toString().padStart(2, 0) +
          "-" +
          endWeek.getDate().toString().padStart(2, 0) +
          "/" +
          (endWeek.getMonth() + 1).toString().padStart(2, 0);
  ...
  .factory("subscriptionsAPI", function ($http, $cookies, settings) {
    ...
    var errorTranslations = {
      ...
      "The given password must be set":
  ...
  .factory("selection", function () {
    ...
    selection.updateFromQueryString = function (queryString) {
      ...
      if (queryString.term !== undefined) {
        this.terms = Array.isArray(queryString.term)
          ? queryString.term
          : [queryString.term];
      } else {
        this.terms = ["*"];
  ...
  .factory("drawers", function (helpers) {
    ...
    drawers.drawEvolutionTotal = function (scope, response) {
      scope.evolution_total = {
        ...
        data: {
          cols: [
            ...
            { type: "number", label: "Todos los temas" },
          ],
          rows: [],
        },
        options: {
          legend: {
            textStyle: { fontName: "Lato", fontSize: 13, bold: false },
          },
          tooltip: {
            textStyle: { fontName: "Lato", fontSize: 13, bold: false },
          },
      ...
      for (i = 0; i < response.evolution.length; i++) {
        ...
        scope.evolution_total.data.rows.push({
          c: [{ v: new Date(row.key - 120 * 1000 * 60) }, { v: row.doc_count }],
        });
      }
      ...
      google.visualization.events.addListener(chart, 'ready', function () {
        ...
        if (svg.length > 0) {
          svg[0].setAttribute('aria-label', 'Gráfica de evolución, a continuación en formato tabla');
        }
      });
    };
    ...
    drawers.drawMultitermEvolution = function (scope, response) {
      ...
      scope.evolution.data.cols.push({
        type: "number",
        label: 'Mensajes con "' + response.evolution[0].term + '"',
      });
    ...
    drawers.drawTotals = function (scope, response) {
      ...
      scope.total_by_source = {
        ...
        data: {
          cols: [
            { type: "string", label: "Fuente" },
            { type: "number", label: "Mensajes" },
          ],
          rows: [],
        },
        options: {
      ...
      scope.total_by_type = {
        ...
        data: {
          cols: [
            { type: "string", label: "Tipo de dato" },
            { type: "number", label: "Mensajes" },
          ],
          rows: [],
        },
    ...
    drawers.drawHemicycles = function (scope, response) {
      ...
      scope.hemicycle_hashtags = {
        options: {
          ...
          tooltip: {
            pointFormat: "Hashtags:  <strong>{point.y} </strong>",
          },
          credits: {
            enabled: false,
          },
      ...
      for (i = 0; i < Math.min(response.top_hashtags.length, 20); i++) {
        ...
        scope.hemicycle_hashtags.series[0].data.push({
          name: row.key,
          y: row.doc_count,
        });
      }
      ...
      scope.hemicycle_mentions = {
        options: {
          ...
          tooltip: {
            pointFormat: "Menciones:  <strong>{point.y} </strong>",
          },
          credits: {
            enabled: false,
          },
      ...
      for (i = 0; i < Math.min(response.top_mentions.length, 20); i++) {
        ...
        scope.hemicycle_mentions.series[0].data.push({
          name: row.key,
          y: row.doc_count,
        });
      }
        ...
        features.addFeature(
          new ol.Feature({
            geometry: new ol.geom.Point(
              ol.proj.transform([row.lon, row.lat], "EPSG:4326", "EPSG:3857")
            ),
            weight: weight,
          })
        );
    ...
    drawers.drawWeeklyCloud = function (scope, response, escuchaAPI, helpers) {
      ...
      scope.timelineSwiper = new Swiper(".timeline .swiper-container-y", {
        ...
        paginationBulletRender: function (swiper, index, className) {
          ...
          // }
          if (!$(".swiper-slide-y")[index]) {
            return;
          }
          var year = $(".swiper-slide-y")[index].getAttribute("data-year");
          ...
          return (
            '<span class="' +
            className +
            ' swiper-pagination-bullet-shown">' +
            year +
            "</span>"
          );
        },
      ...
      Object.keys(response).forEach((year) => {
        ...
        scope["timelineSwiper" + year] = new Swiper(
          ".timeline .swiper-container-" + year,
          {
            direction: "vertical",
            loop: false,
            speed: 1600,
            pagination: ".swiper-pagination-" + year,
            effect: "fade", //Could be "slide", "fade", "cube", "coverflow" or "flip"
            paginationBulletRender: function (swiper, index, className) {
              var activeIndex = 0;
              if (swiper.activeIndex) {
              ...
              // }
              if (!$(".swiper-slide-" + year)[index]) {
                return;
              }
              var week = $(".swiper-slide-" + year)[index].getAttribute(
                "data-week"
              );
              if (activeIndex == index) {
                return (
  ...
  .factory("helpers", function () {
    ...
    helpers.hexToRgb4 = function (hex) {
      var result = /^#?([a-f\d]{2})([a-f\d]{2})([a-f\d]{2})([a-f\d]{2})|([a-f\d]{1})([a-f\d]{1})([a-f\d]{1})([a-f\d]{1})$/i.exec(
        hex
      );
      return result
        ? {
          r: parseInt(
            hex.length <= 4 ? result[4] + result[4] : result[1],
            16
          ),
          g: parseInt(
            hex.length <= 4 ? result[5] + result[5] : result[2],
            16
          ),
          b: parseInt(
            hex.length <= 4 ? result[6] + result[6] : result[3],
            16
          ),
          a: parseFloat(
            parseInt(
              hex.length <= 4 ? result[7] + result[7] : result[4],
              16
            ) / 255.0
          ),
          toString: function () {
    ...
    var accentsMap = {};
    for (var i = 0; i < defaultDiacriticsRemovalap.length; i++) {
      accentsMap[defaultDiacriticsRemovalap[i].base] =
        "[" + defaultDiacriticsRemovalap[i].letters + "]";
    }
    ...
    helpers.createCloud = function (scope, year, escuchaAPI) {
      ...
      var saturdayFormated = new Date(
        new Date(monday).setDate(new Date(monday).getDate() + parseInt(6))
        ...
        .success(function (response) {
          ...
          response.historics_cloud[0]._source.hashtags.forEach((element) => {
            if (element.hashtag.substring(0, 1) == "#") {
              var hashtagText = element.hashtag.substring(
                1,
                element.hashtag.lenght
              );
            } else {
            ...
            if (element.weight_in_list) { newWord.color = "#CD4839" }
