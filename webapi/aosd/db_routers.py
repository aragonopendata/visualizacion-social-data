class MainRouter(object):
    """
    A router to control all database operations on models in the
    main application.
    """
    def db_for_read(self, model, **hints):
        """
        Attempts to read main models go to subscriptions db.
        """
        if model._meta.app_label == 'main':
            return 'subscriptions'
        return None

    def db_for_write(self, model, **hints):
        """
        Attempts to write main models go to subscriptions db.
        """
        if model._meta.app_label == 'main':
            return 'subscriptions'
        return None

    def allow_relation(self, obj1, obj2, **hints):
        """
        Allow relations if a model in the main app is involved.
        """
        if obj1._meta.app_label == 'main' or \
           obj2._meta.app_label == 'main':
           return True
        return None

    def allow_migrate(self, db, app_label, model=None, **hints):
        """
        Make sure the main app only appears in the 'subscriptions db'
        database.
        """
        if app_label == 'main':
            return db == 'subscriptions'
        return None