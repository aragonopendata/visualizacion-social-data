...
class RemoteModel(object):
	...
	def _add_region(self, payload, region):
		if region != '*':
                    if region.upper() in provincia:
                        payload['query']['filtered']['filter']['bool']['must'].append(
                                {
                        "term": {
	...
	def _add_theme(self, payload, query):
		...
		for term in terms:
			if term != '*':
				term = '"' + term + '"'
			payload["query"]["filtered"]["query"]["bool"]["should"].append({
				"query_string": {
	...
	def polarity_mean(self, query='*', region='*', min_published_on=None, max_published_on=None, min_val=None, max_val=None):
		...
		if min_val is not None or max_val is not None:
			...
			payload["query"]["filtered"]["filter"]["bool"]["must"].append(range_polarity)
		self._add_theme(payload, query)
		self._add_region(payload, region)
...
def generate_graph(es_search):
	...
	g.vs['weight'] = g.strength(g.vs, weights=g.es["acum_weight"], mode=igraph.IN)
...
def csv_generator(items):
	...
	writer.writerow(['id', 'source', 'type', 'title', 'description', 'author', 'published_on', 'starts_on', 'ends_on', 'latitude', 'longitude', 'url'])
